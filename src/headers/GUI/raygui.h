...
RAYGUIAPI int GuiTextBoxRounded(Rectangle bounds, char *text, int textSize, bool editMode);              // Text Box control, updates input text
...
int GuiLabelFont(Rectangle bounds, const char *text,Font font,int textSize,int textColor)
{
    int result = 0;
    GuiState state = guiState;
    guiFont=font;
...
int GuiTextBoxRounded(Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    #if !defined(RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN)
        #define RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN  40        // Frames to wait for autocursor movement
    #endif
    ...
    if (state == STATE_PRESSED)
    {
        DrawRectangleRounded(bounds,0.5,0, GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_PRESSED)));
    }
    else if (state == STATE_DISABLED)
    {
        DrawRectangleRounded(bounds,0.5,0,GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_DISABLED)));
    }
    else DrawRectangleRounded(bounds,0.5,1000, WHITE);
    // Draw text considering index offset if required
    // NOTE: Text index offset depends on cursor position
    GuiDrawText(text + textIndexOffset, textBounds, GuiGetStyle(TEXTBOX, TEXT_ALIGNMENT), GetColor(GuiGetStyle(TEXTBOX, TEXT + (state*3))));
